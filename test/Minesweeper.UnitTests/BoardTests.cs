...
public class BoardTests
{
    ...
    [Fact]
    public void Should_ThrowIndexOutOfRangeException_When_SetMineAtIsOutOfBounds()
    {
        var board = new Board(4, 5);

        var act = () => board.SetMineAt(4, 0);

        act.Should().Throw<IndexOutOfRangeException>();
    }

    [Fact]
    public void Should_ThrowIndexOutOfRangeException_When_GetSquareAtIsOutOfBounds()
    {
        var board = new Board(4, 5);

        var act = () => board.GetSquareAt(0, -1);
    ...
    [Fact]
    public void Should_NotCountItself_When_GetTotalAdjacentMinesOnMineSquare()
    {
        var board = new Board(4, 5);
        ...
        board.GetTotalAdjacentMines(1, 2).Should().Be(3);
        board.GetTotalAdjacentMines(2, 2).Should().Be(1);
    }
    ...
    [Fact]
    public void Should_DisplayAllBlanks_When_BoardHasNoMines()
    {
        var board = new Board(2, 3);

        board.Transform().Should().Be("...\n...\n");
    }

    [Fact]
    public void Should_KeepAllMinesDisplayed_When_BoardIsFullOfMines()
    {
    ...
    [Fact]
    public void Should_TransformToBlank_When_SingleSquareBoardHasNoMine()
    {
    ...
    [Fact]
    public void Should_TransformToMine_When_SingleSquareBoardHasMine()
    {
        var board = new Board(1, 1);
    ...
    [Fact]
    public void Should_TransformCorrectly_When_BoardHasSingleRow()
    {
        var board = new Board(1, 3);

        board.SetMineAt(0, 1);

        board.Transform().Should().Be("1*1\n");
    }

    [Fact]
    public void Should_TransformCorrectly_When_BoardHasSingleColumn()
    {
        var board = new Board(3, 1);

        board.SetMineAt(1, 0);

        board.Transform().Should().Be("1\n*\n1\n");
    }
}